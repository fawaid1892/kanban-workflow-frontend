...
): Promise<WorkflowGraph['stages'][number]> {
...
export async function setStageDeps(
  workflowId: string,
  stageId: string,
  parentIds: string[],
): Promise<void> {
  await api.put(`/workflows/${workflowId}/stages/${stageId}/deps`, {
    parentIds,
  });
}
...
export async function fetchWorkflowGraph(
  ...
  const response = await api.get<WorkflowGraph>(
